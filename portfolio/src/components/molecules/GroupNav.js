...
export default function GroupNav() {
    // Definisci uno stato per gestire la visibilità dell'overlay
    const [showOverlay, setShowOverlay] = useState(false);

    // Gestore di eventi per mostrare/nascondere l'overlay
    const toggleOverlay = () => {
        setShowOverlay(!showOverlay);
    };

    return (
        <ul className="flex gap-5 items-center text-white md:gap-20">
            <NavLinks to="/aboutme">About Me</NavLinks>
            <NavLinks to="/skill">Skill</NavLinks>
            {/* Aggiungi un gestore di eventi al componente NavLinks "Contacts" */}
            <button className=' rounded-2xl px-2 border-[1px] border-white  text-[20px] md:border-none md:rounded-none md:text-2xl font-semibold' onClick={toggleOverlay}>Contacts</button>

            {/* Mostra l'overlay condizionalmente in base allo stato */}
            {showOverlay && (
                <div className="overlay">
                    ...
                    <div className='flex justify-center items-center'>
                        <div className='fixed top-0 left-0 px-10 py-10 w-full h-full bg-black bg-opacity-50 flex items-center justify-center'>
                            ...
                            <div className='flex flex-col gap-4 bg-primary font-bold text-white rounded-3xl border-[3px] border-white p-10'>
                                ...
                                <div className='flex gap-3 items-center'>
                                    ...
                                    <p className=' text-2xl'>Whatsapp +39 377 0984469 </p>
                                </div>
                                ...
                                <div className='flex gap-3 items-center'>
                                    ...
                                    <p className=' text-2xl'>Gianlucapieri1593@gmail.com</p>
                                </div>
